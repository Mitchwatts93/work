...
def plot_roc(fpr, tpr, save_path):
    plt.close()
    lw = 2
    roc_auc = auc(fpr, tpr)
...
def plot_all_preds():
    for pred_file in os.listdir(constants.PREDICTIONS_PATH):
        ...
        fpr, tpr, thresholds = roc_curve(y_true=pred_df.truth.values.astype(float), y_score=pred_df.predicted.values)
        plot_roc(fpr, tpr, save_path)
